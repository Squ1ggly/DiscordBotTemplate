import { Collection, EmbedAuthorData, EmbedBuilder, REST, Routes, Message, Interaction, GatewayIntentBits, Client } from "discord.js";
import { IEmbedOptions, IBotHelperClient, ISlashCommand, IPrefixCommand } from "../types/helper-types";
import { readdirSync } from "fs";
import { DateTime } from "luxon";
import path from "path";

export default class DiscordBotHelper {
  ...
  botId: string;
  serverId: string;
  constructor(botImg: string, authorName: string, commandCoolDown: number, commandPrefix: string, botId: string, botToken: string, serverId?: string) {
  ...
  checkSlashCommand(commandMsg: string): boolean {
    const foundCommand = this.client.slashCommandsInfo.find((e: ISlashCommand) => e.name === commandMsg);
    if (foundCommand) {
      return true;
    }
    return false;
  ...
  checkPrefixCommand(commandMsg: string): boolean {
    const foundCommand = this.client.prefixCommandsInfo.find((e: IPrefixCommand) => e.name === commandMsg);
    if (foundCommand) {
      return true;
    }
    return false;
  ...
  setSlashCommands(): void {
    ...
    const commandFiles = readdirSync(upDir + "/SlashCommands").filter((e) => e.endsWith(".ts") || e.endsWith(".js"));
  ...
  setPrefixCommands(): void {
    ...
    const upDir = path.join(__dirname, "../");
    const commandFiles = readdirSync(upDir + "/PrefixCommands").filter((e) => e.endsWith(".ts") || e.endsWith(".js"));
  ...
  async updateDiscordCommands(): Promise<void> {
    ...
    await rest.put(Routes.applicationCommands(this.botId), { body: this.client.prefixCommandsInfo });
  ...
  async clearBotLevelCommands(): Promise<void> {
    const rest = new REST({ version: "10" }).setToken(this.token);
    const curr = (await rest.get(`/applications/${this.botId}/commands`)) as any[];
    if (curr.length > 0) {
  ...
  async updateGuildCommands(): Promise<void> {
    ...
    await rest.put(Routes.applicationGuildCommands(this.botId, this.serverId), { body: [] });
  ...
  startBot(): void {
    ...
    this.client.on(
      ...
      async function (interaction: any) {
        ...
        const coolDown = await this.coolDown(interaction, interaction.commandName);
        if (coolDown === 0) {
          console.log(`User ${interaction.member.user.username} executed command ${interaction.commandName}`);
          await this.client.slashCommands.get(interaction.commandName).execute(interaction, this.client);
        } else {
          await interaction.reply(`Please wait ${coolDown.toFixed(2)} more seconds before using ${interaction.commandName}`);
        }
      }.bind(this)
    );
    ...
    this.client.on(
      ...
      async function (message: any) {
        ...
        const commandText = command.replace(this.commandPrefix, "").split(" ")[0];
        if (!this.checkPrefixCommand(commandText)) {
          message.reply(`Command not recognized, syntax is "${this.commandPrefix}COMMAND INPUT"\n The space is required between the command and the input`);
          return;
        }
        const coolDownMessage = await this.coolDown(message, commandText);
        if (coolDownMessage === 0) {
          console.warn(`User ${message.member.displayName} executed command ${commandText}`);
          await this.client.prefixCommands.get(commandText).execute(message, this.client);
        } else {
          await message.reply(`Please wait ${coolDownMessage.toFixed(2)} more seconds before using ${commandText}`);
        }
      }.bind(this)
    );
